...
TEST_CASE ("STL pair") {
	...
	CHECK(get<0>(m_pair) == 1);

	// 3. Retrieve second data element
	CHECK(get<1>(m_pair) == "apple");
}
...
TEST_CASE ("STL sequence containers") {
	...
	// VECTOR //
	// 7. Declaare a std::vector of string named m_vector
	...
	m_vector.insert(m_vector.begin()+2,"grape");
...
TEST_CASE ("STL sequence container with pair") {
	...
	stringstream ss;
	for (vector<pair<string,float>>::iterator i=m_vector.begin(); i!=m_vector.end(); ++i) {
		ss << get<0>(*i) << ',' << get<1>(*i) << ' ';
	}
