...
void BSTree::inOrder (BSTNode* root, void (*f)(int num)){
    if(root==NULL){
        return;
    }
    if(root->getLeftChild()!=NULL){
...
void BSTree::reverseOrder (BSTNode* root, void (*f)(int num)){
    if(root==NULL){
        return;
    }
    if(root->getRightChild()!=NULL){
