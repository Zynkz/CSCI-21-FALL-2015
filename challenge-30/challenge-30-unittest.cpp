...
void HashTable::insert (Item i){
	bool added=false;
	for(list<Item>::iterator j=items[hash(i)].begin();j!=items[hash(i)].end();++j){
...
bool HashTable::remove (string contents){
	for(array<list<Item>,5>::iterator i=items.begin(); i!=items.end(); ++i){
		for(list<Item>::iterator j=i->begin(); j!=i->end(); ++j){
			if(j->contents==contents){
				i->erase(j);
				return true;
			}
		}
...
unsigned int HashTable::hash (Item i){
	return hash(i.contents)%items.size();
}
...
string HashTable::printDetail (){
	stringstream oss;
	oss<<"Items in hash table: "<<size()<<"\n";
	for(unsigned int i=0;i<items.size();++i){
		oss<<"["<<i<<"]	";
		for(list<Item>::iterator j=items[i].begin();j!=items[i].end();j++){
			oss<<setw(10)<<j->contents<< " ("<<j->count<<") ";
		}
		oss<<"\n";
	}
	return oss.str();
